...
def build_network(images,
	...
	with slim.arg_scope([slim.conv2d, slim.fully_connected],
						...
						net = tf.pad(images, np.array([[0, 0], [1, 1], [1, 1], [0, 0]]), name='pad_1')#[None, 448, 448, 3]  => [None, 454, 454, 3]
						print_activations(net)
						net = slim.conv2d(net, 32, 3, padding='VALID', scope='conv1')    #[None, 448, 448, 3]  => [None, 224, 224, 64]
						print_activations(net)
					
						net = slim.conv2d(net, 64, 3, scope='conv2')    #[None, 112, 112, 64]  => [None, 112, 112, 192]
						print_activations(net)
						net = slim.max_pool2d(net, 2, padding='VALID', scope='pool2')    #[None, 112, 112, 192]  => [None, 56, 56, 192]
						print_activations(net)
						net = slim.conv2d(net, 128, 3, scope='conv3_1')    #[None, 56, 56, 192]  => [None, 56, 56, 128]
						print_activations(net)
						net = slim.conv2d(net, 64, 1, scope='conv3_2')    #[None, 56, 56, 192]  => [None, 56, 56, 256]
						print_activations(net)
						net = slim.conv2d(net, 128, 3, scope='conv3_3')    #[None, 56, 56, 256]  => [None, 56, 56, 256]
						print_activations(net)
						net = slim.conv2d(net, 512, 3, scope='conv_9')    #[None, 448, 448, 3]  => [None, 56, 56, 512]
						print_activations(net)
						net = slim.max_pool2d(net, 2, padding='VALID', scope='pool3')    #[None, 56, 56, 512]  => [None, 28, 28, 512]
						print_activations(net)
						net = slim.conv2d(net, 256, 3, scope='conv4_1')    #[None, 28, 28, 512]  => [None, 28, 28, 256]
						print_activations(net)
						net = slim.conv2d(net, 128, 1, scope='conv4_2')    #[None, 28, 28, 256]  => [None, 28, 28, 512]
						print_activations(net)
						net = slim.conv2d(net, 256, 1, scope='conv4_3')    #[None, 28, 28, 512]  => [None, 28, 28, 256]
						print_activations(net)
						net = slim.max_pool2d(net, 2, padding='VALID', scope='pool4')    #[None, 56, 56, 512]  => [None, 28, 28, 512]
						print_activations(net)
						net = slim.conv2d(net, 512, 3, scope='conv5_1')    #[None, 28, 28, 256]  => [None, 28, 28, 512]
						print_activations(net)
						net = slim.conv2d(net, 256, 1, scope='conv5_2')    #[None, 28, 28, 512]  => [None, 28, 28, 256]
						print_activations(net)
						net = slim.conv2d(net, 512, 3, scope='conv5_3')    #[None, 28, 28, 256]  => [None, 28, 28, 512]
						print_activations(net)
						net = slim.conv2d(net, 256, 1, scope='conv5_4')    #[None, 28, 28, 512]  => [None, 28, 28, 256]
						print_activations(net)
						net = slim.conv2d(net, 512, 3, scope='conv5_5')    #[None, 28, 28, 256]  => [None, 28, 28, 512]
						print_activations(net)
						shortcut = net # 存储这一层特征图，以便后面passthrough层
						net = slim.max_pool2d(net, 2, padding='VALID', scope='pool5')    #[None, 56, 56, 512]  => [None, 28, 28, 512]
						print_activations(net)
						net = slim.conv2d(net, 1024, 3, scope='conv6_1')    #[None, 28, 28, 512]  => [None, 28, 28, 512]
						print_activations(net)
						net = slim.conv2d(net, 512, 1, scope='conv6_2')    #[None, 28, 28, 512]  => [None, 28, 28, 1024]
						print_activations(net)
						net = slim.conv2d(net, 1024, 3, scope='conv6_3')    #[None, 14, 14, 1024]  => [None, 14, 14, 512]
						print_activations(net)
						net = slim.conv2d(net, 512, 1, scope='conv6_4')    #[None, 14, 14, 512]  => [None, 14, 14, 1024]
						print_activations(net)
						net = slim.conv2d(net, 1024, 3, scope='conv6_5')    #[None, 14, 14, 1024]  => [None, 14, 14, 512]
						print_activations(net)
						net = slim.conv2d(net, 1024, 3, scope='conv7_1')    #[None, 14, 14, 512]  => [None, 14, 14, 1024]
						print_activations(net)
						net = slim.conv2d(net, 1024, 3, scope='conv7_2')    #[None, 14, 14, 1024]  => [None, 14, 14, 1024]
						print_activations(net)
						#net = tf.pad(net, np.array([[0, 0], [1, 1], [1, 1], [0, 0]]), name='pad_27')    #[None, 14, 14, 1024]  => [None, 16, 16, 1024]
						print_activations(net)
						net = slim.conv2d(net, 64, 1, scope='conv_shortcut')    #[None, 16, 16, 1024]  => [None, 7, 7, 1024]
						print_activations(net)
						shortcut = reorg(shortcut, 2)
						net = tf.concat([shortcut, net], axis=-1) # channel整合到一起
						print_activations(net)
						predicts = slim.conv2d(net, 425, 1, scope='conv_dec')    #[None, 7, 7, 1024]  => [None, 7, 7, 1024]
...
if __name__ == '__main__':
	...
	init_fn = slim.assign_from_checkpoint_fn("./pretrainded/yolov2_checkpoint_dir/yolo2_coco.ckpt",
        slim.get_variables())
