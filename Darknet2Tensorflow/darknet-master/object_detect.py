...
label_map_filename = "mscoco_label_map.pbtxt"
  ...
  def load_graph(self, frozen_graph):
    ...
    with tf.Graph().as_default() as graph:
        ...
        with tf.variable_scope("coord_transform"):  
            #change the coordinates of (xmin,ymin,xmax,ymax) to ymin, xmin, ymax, xmax for object detection api     
            xmin,ymin,xmax,ymax = tf.unstack(tf.transpose(boxes_tensor))
...
#     image_np = ObjectDetectModel.load_image_into_numpy_array(image)
    ...
    vis_bounding_box(image_np, boxes, scores, classes+1, category_index)
    ObjectDetectModel.save_image_to_image_file(image_np,"detection.jpg")
 

def main():

    MODEL = ObjectDetectModel("models/yolov3_frozen_graph.pb")
    print('model loaded successfully!')
    
    images = tf.gfile.Glob("data"+"/*.jpg")
    for f in images:
        
        run_object_detection(MODEL,f)
