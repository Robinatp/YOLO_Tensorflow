...
tf.app.flags.DEFINE_string('yolov3_ckpt_weight', 'models/yolov3-tiny.ckpt', 'File with yolov3.ckpt names')
tf.app.flags.DEFINE_string('darknet_weights_file', 'weight/yolov3-tiny.weights', 'Binary file with detector weights')
...
class YOLOV3_Tiny(object):
	...
	def __init__(self,weights_file, verbose=False):
		...
		if(FLAGS.save_model_pb):
			self._save_graph_to_file(self.sess, self.sess.graph_def ,"models/yolov3_tiny_frozen_graph.pb") 
...
if __name__ == '__main__':
	...
	yolov3_tiny_net = YOLOV3_Tiny(weights_file=FLAGS.darknet_weights_file) 
	images = tf.gfile.Glob(FLAGS.image_dir+"/*.jpg")
	for f in images:
		yolov3_tiny_net.detect_from_file(image_file=f) 
